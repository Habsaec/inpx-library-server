...
class Loader {
    ...
    async createURL(href, data, type, parent) {
        ...
        this.#blobToManifest.set(url, href)
    ...
    /** After loadReplaced(), links in HTML point at blob: URLs; map back to OPF manifest href. */
    findManifestHrefForBlobUrl(blobUrl) {
        const base = blobUrl.split('#')[0]
        return this.#blobToManifest.get(base)
            ?? (() => {
                for (const [manifestHref, objectUrl] of this.#cache)
                    if (objectUrl === base) return manifestHref
                return null
            })()
    }
    unref(href) {
        ...
        if (count < 1) {
            ...
            const blobUrl = this.#cache.get(href)
            if (blobUrl) {
                URL.revokeObjectURL(blobUrl)
                this.#blobToManifest.delete(blobUrl)
            }
...
const getHTMLFragment = (doc, id) => {
    ...
    return doc.getElementById(id)
        ...
        ?? doc.querySelector(`[xml\\:id="${CSS.escape(id)}"]`)
}

/** Точное совпадение href в манифесте часто ломается (кодировка, префикс пути). */
const findManifestItemByPath = (resources, pathSansHash) => {
    const raw = String(pathSansHash || '').split('#')[0]
    const p = decodeURI(raw).replace(/^\/+/, '').replace(/^\.\//, '')
    const tryHit = (h) => resources.manifest.find(m => m.href === h || decodeURI(m.href) === decodeURI(h))
    let item = tryHit(p) || tryHit(raw)
    if (item) return item
    const file = p.split('/').pop()
    if (!file) return null
    const cands = resources.manifest.filter(m => {
        const tail = m.href.split('/').pop()
        return tail === file || decodeURI(tail) === decodeURI(file)
    })
    if (!cands.length) return null
    if (cands.length === 1) return cands[0]
    const inSpine = cands.filter(c => resources.spine.some(s => s.idref === c.id))
    return inSpine[0] ?? cands[0]
}
...
${doc.querySelector('parsererror').innerText}`)
    ...
    resolveHref(href) {
        ...
        if (path.startsWith('blob:')) {
            const manifestHref = this.#loader.findManifestHrefForBlobUrl(path)
            if (!manifestHref) return null
            path = manifestHref
        }
        const item = findManifestItemByPath(this.resources, path)
            ?? this.resources.getItemByHref(decodeURI(path))
        if (!item) return null
